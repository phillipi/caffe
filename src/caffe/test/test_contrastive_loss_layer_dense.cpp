...
class ContrastiveLossDenseLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  ContrastiveLossDenseLayerTest()
  ...
  virtual ~ContrastiveLossDenseLayerTest() {
...
TYPED_TEST_CASE(ContrastiveLossDenseLayerTest, TestDtypesAndDevices);


// TO DO: replace this with a test that compares densely, e.g., by calling each pixel out to contrastive_loss_layer
TYPED_TEST(ContrastiveLossDenseLayerTest, TestForward) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ContrastiveLossDenseLayer<Dtype> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  // manually compute to compare
  const Dtype margin = layer_param.contrastive_loss_dense_param().margin();
...
TYPED_TEST(ContrastiveLossDenseLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ContrastiveLossDenseLayer<Dtype> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
